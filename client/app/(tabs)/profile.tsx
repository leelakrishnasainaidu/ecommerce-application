      ...
      <ScrollView className='flex-1 px-4' contentContainerStyle={!user ? { flex: 1, justifyContent: 'center', alignItems: 'center' } : { paddingTop: 16 }} showsVerticalScrollIndicator={false}>
        {!user ? (
          ...
          <View className='items-center w-full'>
            <View className='w-24 h-24 rounded-full bg-gray-200 justify-center items-center mb-6'>
              <Ionicons name='person' size={40} color={COLORS.secondary} />
            </View>
            <Text className='text-primary font-bold text-xl mb-2'>Guest User</Text>
            <Text className='text-secondary text-base mb-8 text-center w-3/4 px-4'>Login to view your profile, orders, and more.</Text>
          ...
          <>
            ...
            <View className='items-center mb-8'>
              ...
              {user.publicMetadata?.role === 'admin' && (
                <TouchableOpacity onPress={() => router.push('/admin')} className='bg-primary py-2 px-6 rounded-full mt-4'>
            ...
            {/* Logout Button */}
            <TouchableOpacity className='flex-row items-center p-4 justify-center' onPress={handleLogout}>
